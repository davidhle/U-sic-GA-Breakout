...
# 1 "BallUp.h" 1
# 20 "BallUp.h"
extern const unsigned short BallUpBitmap[100];
# 5 "game.c" 2
# 1 "BallDown.h" 1
# 20 "BallDown.h"
extern const unsigned short BallDownBitmap[100];
# 6 "game.c" 2
# 1 "BallLeft.h" 1
# 20 "BallLeft.h"
extern const unsigned short BallLeftBitmap[100];
# 7 "game.c" 2
# 1 "BallRight.h" 1
# 20 "BallRight.h"
extern const unsigned short BallRightBitmap[100];
# 8 "game.c" 2
...
void updateBall() {
 ...
 if ((!(~(oldButtons)&((1<<0))) && (~buttons & ((1<<0)))) && !ball.started) {
  ball.started = 1;
  ...
  if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5)))
   ...
   && ball.col + ball.width - 1 < 240 - ball.cdel - 5) {
   ball.oldCol = ball.col;
    ball.col += ball.cdel * (3/2);
  }
 } else {
  ...
  if (ball.col <= 1 || ball.col + ball.width-1 >= 240 -2) {
   ball.cdel *= -1;
  }
...
void drawPlayer() {
 ...
 drawRect(player.oldRow, player.oldCol, player.height, player.width, ((31) | (31)<<5 | (31)<<10));
...
void drawBall() {
 ...
 drawRect(ball.oldRow, ball.oldCol, ball.height, ball.width, ((31) | (31)<<5 | (31)<<10));
...
void drawBlock(BLOCK* block) {
 ...
 if(block -> active) {
  ...
  drawRect(block -> row, block -> col, block -> height, block -> width, ((31) | (31)<<5 | (31)<<10));
  block -> erased = 1;
 }
}
